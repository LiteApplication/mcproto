...
gen_serializable_test(
    ...
    validation_fail=[
        (
            ...
            ExcTest(ValueError, re.escape("'chat_mode' must be <= 2: 3")),
        ),
        (
            ("en_GB", 10, 0, True, 0b00100000, 2, False, True),
            ExcTest(ValueError, re.escape("'main_hand' must be <= 1: 2")),
        ),
        (
            ("too long for a locale", 10, 0, True, 0b00100000, 1, False, True),
            ExcTest(ValueError, re.escape("Length of 'locale' must be <= 16: 21")),
        ),
    ],
)
# ServerboundPluginMessage
