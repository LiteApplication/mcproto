...
class ChangeDifficulty(ClientBoundPacket):
    ...
    difficulty: int = field(validator=validators.in_((1, 2, 3)))
    locked: bool = field()
...
class Login(ClientBoundPacket):
    ...
    def serialize_to(self, buf: Buffer) -> None:
        ...
        if self.death_dimension_name is not None and self.death_location is not None:
            buf.write_value(StructFormat.BYTE, True)
            self.death_dimension_name.serialize_to(buf)
            self.death_location.serialize_to(buf)
        else:
            buf.write_value(StructFormat.BYTE, False)
...
class PlayerInfoUpdate(ClientBoundPacket):
    ...
    .. note:: The actions in the list will be sorted in the following order :
    ...
    class _PlayerInfoAction(Serializable, RequiredParamsABCMixin):
        ...
        def validate(self) -> None:
            """Validate that the player info action is correctly constructed."""
    ...
    def __attrs_post_init__(self) -> None:
        # Sort the player actions by ACTION_MASK
        for player_actions in self.player_actions.values():
            player_actions.sort(key=lambda a: a.ACTION_MASK)
...
class PlayerLookAt(ClientBoundPacket):
    ...
    .. note:: The entity ID and entity anchor should be both set or both unset.
    """
...
class Recipe(ClientBoundPacket):
    ...
    action: int = field(validator=validators.in_((0, 1, 2)))
    crafting_recipe_book_open: bool = field()
    crafting_recipe_book_filter_active: bool = field()
    smelting_recipe_book_open: bool = field()
    smelting_recipe_book_filter_active: bool = field()
    blast_furnace_recipe_book_open: bool = field()
    blast_furnace_recipe_book_filter_active: bool = field()
    smoker_recipe_book_open: bool = field()
    smoker_recipe_book_filter_active: bool = field()
    recipe_ids: list[Identifier] = field()
    additional_recipe_ids: list[Identifier] | None = field()
...
class ResourcePackPush(ClientBoundPacket):
    ...
    def _hash_check(self, attribute: Attribute[str], value: str) -> None:
        if (
...
class Respawn(ClientBoundPacket):
    ...
    dimension_name: Identifier = field()
    hashed_seed: int = field()
    game_mode: int = field(validator=validators.in_((0, 1, 2, 3)))
    previous_game_mode: int = field(validator=validators.in_((-1, 0, 1, 2, 3)))
    is_debug: bool = field()
    is_flat: bool = field()
    death_dimension_name: Identifier | None = field()
    death_location: Position | None = field()
    portal_cooldown: int = field()
    data_kept: int = field(validator=validators.in_((0, 1, 2, 3)))
...
class SetDisplayObjective(ClientBoundPacket):
    ...
    position: int = field(validator=[validators.ge(0), validators.le(18)])
...
class SetEntityMotion(ClientBoundPacket):
    ...
    entity_id: int = field()
    velocity_x: int = field(validator=[validators.ge(-32768), validators.le(32767)])
    velocity_y: int = field(validator=[validators.ge(-32768), validators.le(32767)])
    velocity_z: int = field(validator=[validators.ge(-32768), validators.le(32767)])
    ...
    def from_velocity(cls, entity_id: int, velocity: Vec3, tps: float = 20) -> Self:
        ...
        """
        velocity_x = int(velocity.x * 8000 / tps)
        velocity_y = int(velocity.y * 8000 / tps)
        velocity_z = int(velocity.z * 8000 / tps)
        return cls(entity_id=entity_id, velocity_x=velocity_x, velocity_y=velocity_y, velocity_z=velocity_z)

    def get_velocity(self, tps: float = 20) -> Vec3:
        """Get the velocity as a Vec3 in blocks per second."""
        return Vec3(self.velocity_x * tps / 8000, self.velocity_y * tps / 8000, self.velocity_z * tps / 8000)
...
class SetObjective(ClientBoundPacket):
    ...
    mode: int = field(validator=validators.in_((0, 1, 2)))
    objective_value: TextComponent | None = field()
    objective_type: int | None = field()
    number_format: int | None = field()
    number_format_content: TextComponent | NBTag | None = field()
...
class SetScore(ClientBoundPacket):
    ...
    def __atrrs_post_init__(self) -> None:
        if self.number_format is not None:
