...
from mcproto.packets.packet import ClientBoundPacket, GameState
...
class FinishConfiguration(ClientBoundPacket):
    ...
    The client answers with :class:`FinishConfiguration` whenever it is ready to continue.
...
@final
@define
class SelectKnownPacks(ClientBoundPacket):
    """Informs the client of which data packs are present on the server. (Server -> Client).

    The client is expected to respond with its own :class:`ServerboundSelectKnownPacks` packet. The Notchian server does not
    continue with Configuration until it receives a response.

    Initialize the ClientboundSelectKnownPacks packet.
